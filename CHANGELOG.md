...
- 修复 `UpdateScreenClamp` Postfix 对 CameraAspect 装饰无条件设置 `clampToScreen = true`，导致部分装饰在播放时一直显示的问题。现在改为仅在编辑器环境且装饰已启用屏幕裁剪时修正位置，不再强制覆写裁剪状态。
...
- 高级渲染设置新增音频格式选项：
  - AAC：有损压缩，兼容性最好（默认）。
  - FLAC：无损压缩。
  - ALAC：无损压缩（Apple Lossless，MP4 原生支持）。
- 渲染进度窗口新增中文性能信息：
  - 单帧内存占用。
  - 缓存上限。
  - GPU 回读队列上限。
